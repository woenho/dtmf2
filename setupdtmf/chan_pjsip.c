...
static void transfer_refer(struct ast_sip_session *session, const char *target)
{
    ...
    // add by woenho on 2021-12-16
    const char* value = pbx_builtin_getvar_helper(chan, "REFER_HEADER");
    if (value){
        char* headers = ast_strdup(value);
        char* next = headers;
        char* header;
        char* val;
        char* p;
        do{
            ...
            if (val){
                ast_sip_add_header(packet, header, val);
            }
        }while(next);
        ast_free(headers);
    }
